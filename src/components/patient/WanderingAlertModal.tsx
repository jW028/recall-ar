...
import type { WanderingState } from '@/viewmodels/useWanderingViewModel';
import { useEffect } from 'react';
import { Modal, Pressable, StyleSheet, Text, Vibration, View } from 'react-native';

interface WanderingAlertModalProps {
    visible: boolean;
...
export function WanderingAlertModal({
    visible,
    countdownSeconds,
    wanderingState,
    onCancel,
    onImmediateSOS,
}: WanderingAlertModalProps) {
    const theme = useTheme();

    useEffect(() => {
        if (visible && wanderingState === 'countdown') {
            ...
            <View style={styles.overlay}>
                <View style={[styles.card, { backgroundColor: theme.cardBackground, borderColor: theme.error }]}>
                    <View style={[styles.iconContainer, { backgroundColor: theme.error + '20' }]}>
                        <Ionicons
                            name={wanderingState === 'triggered' ? 'location-sharp' : 'navigate-circle-outline'}
                            size={56}
                            color={theme.error}
                        />
                    </View>
                    ...
                    <Text style={[styles.title, { color: theme.heading }]}>
                        {wanderingState === 'triggered'
                            ? 'Wandering Alert Dispatched'
                            : 'Outside Safe Zone'}
                    </Text>
                    ...
                    <Text style={[styles.message, { color: theme.textMuted }]}>
                        {wanderingState === 'triggered'
                            ...
                            : `You have been outside your safe zone for 15 minutes. Are you OK?\nAlerting caregiver in ${countdownSeconds}s...`}
                    </Text>

                    {wanderingState === 'countdown' && (
                        ...
                        {wanderingState === 'countdown' ? (
                            <>
                                <Pressable
                                    ...
                                    onPress={onCancel}
                                >
                                    <Text style={styles.cancelButtonText}>I'm OK - Dismiss</Text>
                                </Pressable>
