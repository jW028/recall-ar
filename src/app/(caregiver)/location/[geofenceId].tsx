...
export default function GeofenceConfigScreen() {
    const { geofenceId } = useLocalSearchParams<{
        id: string;
        geofenceId: string;
    }>();
    ...
    const {
        ...
        updateError,
        clearUpdateError,
    } = useGeofenceConfigViewModel(geofenceId);
    ...
    // ── Prefill form when geofence loads ─────────────────────────
    useEffect(() => {
        if (!geofence) return;
        setLatitude(geofence.centerLatitude.toFixed(6));
        setLongitude(geofence.centerLongitude.toFixed(6));
        setRadius(String(geofence.radiusMeters));
        setZoneType(geofence.geofenceType);
    }, [geofence]);
    ...
    if (isLoading) {
        return (
            <Screen background="page">
                <ScreenHeader title="Safe Zone" showBack />
    ...
    // ── Initial map region ────────────────────────────────────────
    const initialRegion = geofence
        ? regionForRadius(geofence.centerLatitude, geofence.centerLongitude, geofence.radiusMeters)
        : { latitude: 3.139, longitude: 101.6869, latitudeDelta: 0.05, longitudeDelta: 0.07 };
        ...
        <Screen background="page">
            ...
            >
                ...
                {events.length === 0 ? (
                    ...
                    events.map(item => {
                        const isEnter = item.eventType === 'Enter';
                        return (
                            <View key={item.geoEventId} style={styles.eventRow}>
                                <View style={[
                                    styles.eventBadge,
                                    isEnter ? styles.enterBadge : styles.exitBadge,
                                ]}>
...
function createStyles(theme: Theme) {
    return StyleSheet.create({
        ...
        enterBadge: { backgroundColor: '#DCFCE7' },
        exitBadge: { backgroundColor: '#FEF3C7' },
        eventBadgeText: {
            fontSize: 12,
            fontWeight: '700',
            color: '#374151',
